...
function validateHeaderChunk(chunk) {
  const normalizedHeaders = chunk.map(header => utils.normalizeHeader(header));

  // verifying timestamps if possible (ts > avg of previous 11 timestamps)
  ...
  const badHeaders = invalidTimestamp.concat(invalidPOW, invalidTarget, badConnections);
