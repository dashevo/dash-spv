  ...
  addHeaders(headers) {
    ...
    const allValid = normalizedHeaders.reduce(
      (acc, header, index, array) => {
        const previousHeaders = array.slice(0, index);
        if (index !== 0) {
        ...
        if (!self.isValid(header, previousHeaders)) {
          throw new Error('Some headers are invalid');
        }
        if (!SpvChain.isParentChild(header, this.getTipHeader())) {
          isOrphan = true;
        }
        return acc && true;
      }, true,
    ...
    if (isOrphan) {
      ...
      self.appendHeadersToLongestChain(headers);
    }
    this.checkPruneBlocks();
  }
};
