...
const SpvChain = class {
  ...
  appendHeaderToLongestChain(header) {
    const longestChain = this.getLongestChain();
    longestChain.push(header);
    this.allBranches = [];
    this.allBranches.push(longestChain);
  }
  ...
  validateParentChild(header, previousHeader) {
    if (utils.getCorrectedHash(header.prevHash) !== previousHeader.hash) {
      return false;
    }
    if (Consensus.isValidBlockHeader(header, this.getLongestChain(), this.network)
      ...
      this.appendHeaderToLongestChain(header);
      return true;
    }
    return false;
  }
  ...
  addHeaders(headers) {
    ...
    const allValidated = normalizedHeaders.reduce(
      (acc, header, index, array) => {
        if (index === 0) {
          return acc && self.validateParentChild(header, this.getTipHeader());
        }
        return acc && self.validateParentChild(header, array[index - 1]);
      }, true,
    );
    if (!allValidated) {
