...
const SpvChain = class {
  ...
  appendHeadersToLongestChain(headers) {
    ...
    headers.forEach((header, index) => {
      const { hash } = header;
      this.headersHeights.set(hash, longestChain.length + index);
    });
  ...
  addHeader(header) {
    ...
    const longestChain = this.getLongestChain();
    if (this.isValid(headerNormalised, longestChain)) {
      headerNormalised.children = [];
      this.processValidHeader(headerNormalised);
      this.setAllBranches();
      this.checkPruneBlocks();
      this.headersHeights[header.hash] = longestChain.length;
      return true;
    }
    return false;
  }
   ...
   */
  addHeaders(headers) {
    console.log(`Add ${headers.length} headers`);
    const now = Date.now();
    // TODO: throws stack overflow error for some reason. Treat one header same as many
    // if (headers.length === 1) {
    ...
    // Allow intersection of 1 block
    if (tip.hash === normalizedHeaders[0].hash) {
      normalizedHeaders.splice(0, 1);
    }

    // TODO: pay attention when you are removing the only header in the batch.
    // Is it okay?
    const isOrphan = normalizedHeaders.length
      && !SpvChain.isParentChild(normalizedHeaders[0], tip);
    ...
    console.log(`Added ${headers.length} headers for ${passed / 1000}sec`);
