...
const SpvChain = class {
  ...
  addHeader(header, network = 'mainnet') {
    const headerNormalised = utils.normalizeHeader(header);

    if (Consensus.isValidBlockHeader(headerNormalised, this.getLongestChain(), network)
  ...
  addHeaders(headers, network = 'mainnet') {
    const self = this;
    const allAdded = headers.reduce((acc, header) => acc && self.addHeader(header, network), true);
